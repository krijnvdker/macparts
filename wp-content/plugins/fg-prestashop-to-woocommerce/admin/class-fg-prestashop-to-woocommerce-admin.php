...
if ( !class_exists('FG_PrestaShop_to_WooCommerce_Admin', false) ) {
	...
	class FG_PrestaShop_to_WooCommerce_Admin extends WP_Importer {
		...
		public $shop_id = 1;						// Shop ID to import
...
SQL;
		...
		private function get_cms_count() {
			...
			if ( version_compare($this->prestashop_version, '1.5', '>=') ) {
				// PrestaShop 1.5+
				$extra_joins .= "INNER JOIN ${prefix}cms_lang AS cl ON cl.id_cms = c.id_cms";
				$extra_joins .= " INNER JOIN ${prefix}cms_shop AS cs ON cs.id_cms = c.id_cms";
				$extra_criteria .= "WHERE cl.id_lang = '{$this->current_language}'";
				$extra_criteria .= " AND cs.id_shop = '{$this->shop_id}'";
				if ( $this->column_exists('cms_lang', 'id_shop') ) {
					$extra_criteria .= " AND cl.id_shop = '{$this->shop_id}'";
				}
			}
			$sql = "
				SELECT COUNT(*) AS nb
		...
		private function get_cms_categories_count() {
			...
			if ( version_compare($this->prestashop_version, '1.6', '>=') ) {
				// PrestaShop 1.6+
				$extra_joins .= "INNER JOIN ${prefix}${category_table}_lang cl ON cl.id_${category_table} = c.id_${category_table}";
				$extra_criteria .= " AND cl.id_lang = '{$this->current_language}'";
						...
						foreach ( $category_thumbnails as $category_thumbnail ) {
							if ( !empty($category_thumbnail) ) {
								...
								if ( !empty($thumbnail_id) ) {
									$this->media_count++;
									update_woocommerce_term_meta($new_category_id, 'thumbnail_id', $thumbnail_id);
		...
		public function import_product($product, $language) {
			...
			if ( $new_post_id ) {
				...
				if ( version_compare($this->prestashop_version, '1.5', '<') ) {
					...
					$stock = $this->get_product_stock($product['id_product'], $this->shop_id);
					if ( !empty($stock) ) {
		...
		protected function get_specific_prices($product_id) {
			...
			if ( version_compare($this->prestashop_version, '1.4', '>=') ) {
				...
				$sql = "
					SELECT $product_attribute_column, p.price, p.reduction, $reduction_tax_field, p.reduction_type, p.`from`, p.`to`
		 ...
		 * @return array Stock
		 */
		protected function get_product_stock($product_id, $shop_id) {
			$stock = array();
			$prefix = $this->plugin_options['prefix'];
			$sql = "
				...
				AND (s.id_shop = '$shop_id' OR s.id_shop = 0)
		...
		private function get_cms_categories() {
			...
			if ( $this->table_exists($category_table) ) {
				...
				$sql = "
					...
					FROM ${prefix}${category_table} c
					INNER JOIN ${prefix}${category_table}_lang cl ON cl.id_${category_table} = c.id_${category_table} AND cl.id_lang = '{$this->current_language}'
					WHERE c.active = 1
					$extra_criteria
					ORDER BY $order
				";
		...
		protected function get_cms_articles($limit=1000) {
			...
			if ( version_compare($this->prestashop_version, '1.4', '<') ) {
				...
				if ( version_compare($this->prestashop_version, '1.5', '>=') ) {
					...
					$extra_criteria .= " AND cs.id_shop = '{$this->shop_id}'";
					if ( $this->column_exists('cms_lang', 'id_shop') ) {
						$extra_criteria .= " AND l.id_shop = '{$this->shop_id}'";
					}
				}
				$sql = "
		...
		private function get_all_product_categories() {
			...
			$sql = "
				SELECT c.id_category, c.date_add AS date, $position_field, c.id_parent, $root_category_field, cl.name, cl.description, cl.link_rewrite AS slug, cl.meta_description, cl.meta_keywords, cl.meta_title
				FROM ${prefix}category c
		...
		private function get_products($limit=1000) {
			...
			if ( version_compare($this->prestashop_version, '1.5', '<') ) {
				...
				$sql = "
					SELECT p.id_product, p.id_supplier, p.id_manufacturer, $id_tax_rules_group, p.id_category_default, p.on_sale, p.quantity, p.price, p.wholesale_price, p.reference, p.ean13, p.supplier_reference, $location_field, $width_field, $height_field, $depth_field, p.weight, p.out_of_stock, p.active, $available_for_order_field, 'both' AS visibility, 0 AS is_virtual, p.date_add AS date, pl.name, pl.link_rewrite AS slug, pl.description, pl.description_short, pl.meta_description, pl.meta_keywords, pl.meta_title
					$reduction_fields
					FROM ${prefix}product p
				...
				// PrestaShop 1.5+
				$sql = "
					SELECT DISTINCT p.id_product, p.id_supplier, p.id_manufacturer, ps.id_tax_rules_group, ps.id_category_default, ps.on_sale, 0 AS quantity, ps.price, ps.wholesale_price, p.reference, p.ean13, p.supplier_reference, $location_field, p.width, p.height, p.depth, p.weight, 0 AS out_of_stock, ps.active, ps.available_for_order, ps.visibility, p.is_virtual, ps.date_add AS date, pl.name, pl.link_rewrite AS slug, pl.description, pl.description_short, pl.meta_description, pl.meta_keywords, pl.meta_title
					FROM ${prefix}product p
					INNER JOIN ${prefix}product_lang AS pl ON pl.id_product = p.id_product AND pl.id_lang = '$lang' AND pl.id_shop = '{$this->shop_id}'
					INNER JOIN ${prefix}product_shop AS ps ON ps.id_product = p.id_product AND ps.id_shop = '{$this->shop_id}'
				";
			}
			$sql .= "
		...
		protected function get_product_images($product_id) {
			...
			$sql = "
				SELECT i.id_image, i.position, $cover, il.legend
				FROM ${prefix}image i
...
//					$this->display_admin_error("Can't copy $old_filename to $new_full_filename : $error_message");
		...
		public function remote_copy($url, $path) {
			...
			if ( is_wp_error($response) ) {
				...
				$real_extension = pathinfo($response_object->url, PATHINFO_EXTENSION);
				if ( preg_match('/^text/', $content_type) ) {
					// Not a media
					trigger_error('Not a media', E_USER_WARNING);
				} elseif ( $extension != $real_extension ) {
					// Redirected to a file with another extension
					trigger_error('Redirected to a file with a different extension', E_USER_WARNING);
				} else {
